...
# ---------------------------------------------------------------------------
# Data loading from local files in data/yoy/
# ---------------------------------------------------------------------------
# Expected files (already in PST, produced by download_data.py):
#   data/yoy/lmp_NP15_YYYY.csv   columns: timestamp, lmp
#   data/yoy/lmp_SP15_YYYY.csv   columns: timestamp, lmp
#   data/yoy/solar_YYYY.csv      columns: timestamp, solar_mw

def load_lmp(year: int, hub: str) -> pd.DataFrame:
    """Load local LMP CSV for a given year and hub (NP15 or SP15)."""
    path = DATA_DIR / f"lmp_{hub}_{year}.csv"
    if not path.exists():
        raise FileNotFoundError(
            f"Missing: {path}\n"
            f"Run download_data.py for January {year} and save output to data/yoy/"
        )
    df = pd.read_csv(path)
    df["timestamp"] = pd.to_datetime(df["timestamp"])
    df = df.rename(columns={"lmp": f"{hub.lower()}_lmp"})
    return df.sort_values("timestamp").drop_duplicates("timestamp").reset_index(drop=True)


def load_solar(year: int) -> pd.DataFrame:
    """Load local solar generation CSV for a given year."""
    path = DATA_DIR / f"solar_{year}.csv"
    if not path.exists():
        raise FileNotFoundError(
            f"Missing: {path}\n"
            f"Run download_data.py for January {year} and save output to data/yoy/"
...
def compute_yoy_metrics(years: list) -> dict:
    ...
    for year in years:
        print(f"\n  Year {year}:")
        try:
            sp15_df  = load_lmp(year, "SP15")
            np15_df  = load_lmp(year, "NP15")
            solar_df = load_solar(year)
...
.meta{font-size:13px;color:#9a9890;margin-bottom:6px}
.thesis{background:#111;border:1px solid #1e1e1e;border-radius:8px;padding:13px 16px;margin-bottom:20px;font-size:14px;color:#c8c5be;line-height:1.65}
.thesis strong{color:#d3d1c7;font-weight:500}
.controls{display:flex;gap:8px;margin-bottom:20px;align-items:center}
.ctrl-lbl{font-size:12px;letter-spacing:1px;color:#9a9890;text-transform:uppercase}
.seg{display:flex;border:1px solid #222;border-radius:5px;overflow:hidden}
.seg-btn{background:none;border:none;border-right:1px solid #222;color:#b0aea8;font-size:13px;padding:5px 12px;cursor:pointer;font-family:inherit}
...
.kpi-year{font-size:12px;letter-spacing:0.8px;color:#9a9890;text-transform:uppercase;margin-bottom:5px}
.kpi-val{font-size:20px;font-weight:500;color:#f0ede8;line-height:1}
.kpi-sub{font-size:13px;margin-top:4px;color:#b0aea8}
...
.callout{border-left:2px solid #185fa5;background:#091420;border-radius:0 4px 4px 0;padding:8px 12px;margin-top:10px;font-size:13px;color:#b0aea8;line-height:1.6}
...
def main():
    ...
    # Discover available years from files in data/yoy/
    years = sorted({
        int(p.stem.split("_")[-1])
        for p in DATA_DIR.glob("solar_*.csv")
    })
    print(f"Years found in data/yoy/: {years}\n")
