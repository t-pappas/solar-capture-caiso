...
.header-left .eyebrow{font-size:12px;letter-spacing:1.8px;color:#9a9890;text-transform:uppercase;margin-bottom:6px}
.header-left h1{font-size:22px;font-weight:500;color:#f0ede8;letter-spacing:-0.3px}
.header-left .meta{font-size:13px;color:#9a9890;margin-top:4px}
.controls{display:flex;gap:8px;flex-wrap:wrap;align-items:center}
.ctrl-label{font-size:12px;letter-spacing:1px;color:#9a9890;text-transform:uppercase}
.seg-group{display:flex;border:1px solid #222;border-radius:5px;overflow:hidden}
.seg-btn{background:none;border:none;border-right:1px solid #222;color:#b0aea8;font-size:13px;padding:5px 12px;cursor:pointer;font-family:inherit;transition:background 0.1s,color 0.1s}
...
.exec-title{font-size:12px;letter-spacing:1.2px;color:#9a9890;text-transform:uppercase;margin-bottom:10px}
.exec-list{list-style:none;display:flex;flex-direction:column;gap:7px}
.exec-list li{font-size:14px;color:#c8c5be;padding-left:14px;position:relative;line-height:1.55}
...
.red{color:#e24b4a}.amber{color:#ef9f27}.green{color:#639922}.muted{color:#9a9890}
...
.fig-eyebrow{font-size:13px;letter-spacing:1.4px;color:#8a8880;text-transform:uppercase;margin-bottom:4px}
.fig-title{font-size:14px;font-weight:500;color:#c8c5be;margin-bottom:3px}
...
.leg{display:flex;align-items:center;gap:5px;font-size:12px;color:#b0aea8}
.leg-sq{width:10px;height:10px;border-radius:2px;flex-shrink:0}
.leg-line{width:18px;height:2px;border-radius:1px;flex-shrink:0}
.chart-wrap{position:relative}
.callout{background:#0d1a0d;border-left:2px solid #3b6d11;border-radius:0 4px 4px 0;padding:8px 12px;margin-top:10px;font-size:13px;color:#b0aea8;line-height:1.6}
...
.hm-hlabel{font-size:8.5px;color:#8a8880;text-align:center;padding:2px 1px}
.hm-dlabel{font-size:13px;color:#9a9890;padding-right:8px;text-align:right;white-space:nowrap;vertical-align:middle;min-width:28px}
.legend-bar{display:flex;align-items:center;gap:8px;margin-top:10px;font-size:12px;color:#9a9890}
...
let charts = {};
const GRID = '#1a1a1a';
const TICK = {color:'#8a8880', font:{size:12}};
  ...
  charts['c1'] = new Chart(document.getElementById('c1'), {
    ...
    options: {
      ...
      scales:{
        ...
        y:{position:'left',ticks:{...TICK,callback:v=>v<0?`-$${Math.abs(v)}`:`$${v}`},grid:{color:GRID},title:{display:true,text:'$/MWh',color:'#8a8880',font:{size:11}}},
        y2:{position:'right',ticks:{...TICK,callback:v=>`${v}GWh`},grid:{display:false},title:{display:true,text:'GWh/day',color:'#27500a',font:{size:11}}},
      }
    }
  });
  const k = d().kpis[hub];
...
function updateC4() {
  ...
  charts['c4'] = new Chart(document.getElementById('c4'),{
    ...
    options:{
      ...
      scales:{
        x:{title:{display:true,text:'Solar generation (GW)',color:'#8a8880',font:{size:12}},ticks:{...TICK,callback:v=>`${v}GW`},grid:{color:GRID}},
        y:{title:{display:true,text:hub.toUpperCase()+' LMP ($/MWh)',color:'#8a8880',font:{size:12}},ticks:{...TICK,callback:v=>v<0?`-$${Math.abs(v)}`:`$${v}`},grid:{color:GRID}},
      }
    }
  });
